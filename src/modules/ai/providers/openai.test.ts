...
function createProvider(): OpenAIChatProvider {
...
describe("OpenAIChatProvider", () => {
  ...
  it("lanza error si falta la API key", () => {
    expect(
      () => new OpenAIChatProvider({ apiKey: "", model: "gpt-4o-mini" })
  ...
  it("complete lanza error si la petición falla", async () => {
    ...
    ).rejects.toThrow(expect.objectContaining({ code: "OPENAI_REQUEST_FAILED" }));
  });
